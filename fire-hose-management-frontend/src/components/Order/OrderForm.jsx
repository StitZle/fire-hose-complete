...
const OrderForm = () => {
    ...
    const departmentSelection = () => {
        let selectableDepartments = departments.map( ( department, index ) => (<MenuItem key={index} value={department.id}>
            {department.departmentName}
        </MenuItem>) );

        selectableDepartments.push( <MenuItem key={selectableDepartments.length + 1} value={"NO_DEPARTMENT"}>
    ...
    const primaryDevices = showDevices.map( ( device, index ) => {
        return (<Grid item lg={12} md={12} xs={12} key={index}>
            <CTextField
                    required={false}
                    type={"number"}
    ...
    const secondaryDevices = nonShowDevices.map( ( device, index ) => (<MenuItem key={index} value={device}>
        {device.deviceName}
    </MenuItem>) );
    ...
    return (<DefaultPage>
        {departments.length === 0 && devices.length === 0 && (<Grid container>
        ...
        {/*TODO show this in the middle of the screen*/}
        {departments.length !== 0 && devices.length !== 0 && (<ValidatorForm onSubmit={handleSubmit}>
            <Grid container spacing={3} className={classes.form}>
                ...
                {nonShowDevices.length !== 0 && (<Grid item lg={12} md={12} xs={12}>
                    <CSelect
                            value={""}
