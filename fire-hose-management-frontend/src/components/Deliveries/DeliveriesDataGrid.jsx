...
const DeliveriesDataGrid = ({ deliveries = [] }) => {
    ...
    const checkboxRender = ( params ) => {
        ...
        return <Checkbox checked={checked} disabled={true} color="primary"/>;
    };
    ...
    const deleteUser = React.useCallback( ( row ) => () => {
    ...
    const columns = React.useMemo( () => [{
        ...
        type: "actions",
        width: 80,
        getActions: ( params ) => [<GridActionsCellItem icon={<SearchIcon/>} label="Details" onClick={deleteUser( params.row )}/>,],
    },], [deleteUser] );
    ...
    return (<div className={classes.dataGrid}>
        <Typography>Abgaben für das Jahr: {dayjs().year()}</Typography>
